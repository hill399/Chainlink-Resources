...
contract ChainlinkClient {
  ...
  uint256 constant internal LINK = 10**17;          /* Mod for 0.1 LINK increments */
...
contract HeartBeatContract is ChainlinkClient, Ownable {
  using SafeMath for uint256;

  uint256 constant private ORACLE_PAYMENT = 1 * LINK; /* LINK constant changed to allow 0.1 LINK payments (li 671) */
  uint256 private cycleNo = 0;
  ...
  address public cronScript;
  string public bytes32JobId;
  string public int256JobId;
  string public uint256JobId;
  ...
  modifier onlyCronScript() {
      require(msg.sender == cronScript);
      _;
  }
  
  function configureOracle(address _oracleAddress, address _cronScript, string _bytes32JobId, string _int256JobId, string _uint256JobId) 
   public
   onlyOwner
  {
      cronScript = _cronScript;
      oracleAddress = _oracleAddress;
      bytes32JobId = _bytes32JobId;
      int256JobId = _int256JobId;
      uint256JobId = _uint256JobId;

      emit oracleConfigured(oracleAddress, cronScript, bytes32JobId, int256JobId, uint256JobId);
  }
  ...
  function nodeHeartbeat()
   ...
   onlyCronScript
  {
    cycleNo = cycleNo.add(1);
    uint modCycle = cycleNo.mod(3);
    if(modCycle == 0){
        requestEthereumPrice(oracleAddress, uint256JobId);
    } else if (modCycle == 1){
        requestEthereumChange(oracleAddress, int256JobId);
    } else if (modCycle == 2){
        requestEthereumLastMarket(oracleAddress, bytes32JobId); 
    }

    emit heartBeatTriggered(cycleNo, modCycle, block.timestamp);
  }
