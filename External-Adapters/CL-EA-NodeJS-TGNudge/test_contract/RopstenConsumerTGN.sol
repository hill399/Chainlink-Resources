...
contract ARopstenConsumer is Chainlinked, Ownable {
  ...
  bytes32 public messageStatus;

  event RequestTGNMessage(
    bytes32 indexed requestId,
    bytes32 indexed chat_id,
    bytes32 indexed text_input
  );
  

  constructor() Ownable() public {
      setLinkToken(0x20fE562d797A42Dcb3399062AE9546cd06f63280);
      setOracle(ENTER_ORACLE_ADDRESS_HERE);
  }

  function requestTGNMessage(string _jobId, string _chat_id, string _text_input) 
    public
    onlyOwner
  {
    Chainlink.Request memory req = newRequest(stringToBytes32(_jobId), this, this.fulfillTGNMessage.selector);
    req.add("endpoint", "sendMessage");
    req.add("chat_id", _chat_id);
    req.add("text_input", _text_input);
    chainlinkRequest(req, ORACLE_PAYMENT);
  }

  function fulfillTGNMessage(bytes32 _requestId, bytes32 _chat_id, bytes32 _text_input)
    public
    recordChainlinkFulfillment(_requestId)
  {
    emit RequestTGNMessage(_requestId, _chat_id, _text_input);
    messageStatus = "Sent";
  }  
